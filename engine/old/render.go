...
type TemplateCtx = engine.TemplateCtx

var PAGE_FILE = engine.PAGE_FILE
var ROOT_DIR = engine.ROOT_DIR
...
func (r *Render) Html(template_string string) (string, error) {
	g_start := time.Now()
	fmt.Println("internal timer ", time.Since(g_start))
	// root variables
	var tr = strings.NewReader(template_string)
	fmt.Println(":cost NewReader ", time.Since(g_start))
	...
	// var nested = 0
	//
	fmt.Println(":cost Variables Init ", time.Since(g_start))

	// Start core loop
	for ; index < length; index++ {
		if err != nil {
			if errors.Is(err, io.EOF) { // preferred way by GoLang doc
		...
		// ROOT CASE
		case '<':
			fmt.Println("TAG NAME: ", ReadWhile(tr, unicode.IsLetter))
		// ROOT CASE
		case '{':
...
func ReadWhile(r *strings.Reader, fn func(rune) bool) string {
	var result strings.Builder
	for {
