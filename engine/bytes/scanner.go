...
func WispyScan(data []byte, atEOF bool) (advance int, token []byte, err error) {
	...
	for width := 0; start < d_len; start += width {
		...
		next_r, next_width := utf8.DecodeRune(data[start+width:])
		if (next_r == '{' && next_width == '{') || !isSpace(r) {
			break
		}
	}
	...
	for width, i := 0, start; i < d_len; i += width {
		...
		if r == '<' && IsCapital(next_r) && len(component_name) == 0 {
			fmt.Println("[", color.CyanString(string(next_r)), "]")
		...
		if len(component_name) > 0 {
			...
			name_complete := component_name[len(component_name)-1] == ' '
			if name_complete {
				tag_len := len(component_name)
				if i > tag_len {
					// logic for checking for nested components of the same type
					test := data[i-(tag_len+1) : i]
					has_suffix := bytes.HasSuffix(test, component_name[:tag_len-1])
					if has_suffix {
						fmt.Println(color.RedString("nested_++"), string(test), color.GreenString("vs"), string(component_name))
			...
			if r == '>' {
				...
				tag_len := len(component_name)
				test := data[i-(tag_len+1) : i]
				closing_tag := append([]byte{'<', '/'}, component_name[1:tag_len-1]...)
				has_suffix := bytes.HasSuffix(test, closing_tag)
				if has_suffix {
					if nested_component_lvl > 0 {
						nested_component_lvl -= 1
						fmt.Println(color.GreenString(string(test)), fmt.Sprint(has_suffix), color.CyanString(string(component_name[:tag_len-1])))
					} else {
			...
			fmt.Println(string(next_r))
			if r == '{' {
				color.Cyan("Hey I'm saving a Bracket!!")
				fmt.Println(string(data[start : i+width]))
			...
			if next_r == '<' {
				...
				return i + next_width + 1, data[start : i+next_width+1], nil
			}
		}
	}
