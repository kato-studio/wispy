...
/* === Change Log
- Updated imports to used required internal packages (also copied required internal packages to local lib)
- added "executeComponent" method
-
-
-
*/
...
import (
	...
	"github.com/kato-studio/wispy/internal/fmtsort"
	"github.com/kato-studio/wispy/internal/text/template/parse"
)

func (s *state) executeComponent(c *parse.ComponentNode, value reflect.Value) {
	...
	slotState.walk(c.Slot) // Use the aliased parse.Node type

	// Prepare arguments for the component function
	args := []reflect.Value{
		reflect.ValueOf(c.Name),
		value,
		reflect.ValueOf(template.HTML(slotContent.String())),
	}

	// Call the component function
	fn := s.evalFunction("component", args)
	result := fn.Call(args)

	// Write the result
	s.wr.WriteString(result[0].String())
}

func (s *state) evalFunction(name string, args []reflect.Value) reflect.Value {
	fn := s.tmpl.Funcs[name]
	if fn == nil {
		s.errorf("function %s not defined", name)
	}
	return reflect.ValueOf(fn)
}

func (s *state) write(str string) {
	s.wr.WriteString(str)
}
