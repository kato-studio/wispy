...
/* === Change Log
- Added new "ComponentNode" struct
-
-
-
-
...
// ComponentNode represents a component with slot content.
type ComponentNode struct {
	NodeType
	Pos
	Name string    // Component name (e.g., "navbar")
	Args []Node    // Arguments passed to the component
	Slot *ListNode // Captured slot content
}

// Modify the parser to recognize {{component}} and capture content until {{end}}:
func (p *Parser) parseComponent() Node {
	const keyword = "component"
	// Parse component name and arguments
	name, args := p.parseCommand()
	// Capture content until {{end}}
	slot := p.parseList(endKeyword)
	p.expect(endKeyword, keyword)
